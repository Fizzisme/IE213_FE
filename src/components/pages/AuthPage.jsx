...
const SLIDES = [
  {
    ...
    desc: 'Nâng tầm ảnh hưởng của bạn trong ngành y tế. Cải thiện chăm sóc bệnh nhân thông qua kiểm soát dữ liệu chính xác, đặt lịch hẹn liền mạch và quản lý công việc hiệu quả.',
    img: 'https://images.unsplash.com/photo-1758573467051-71613f7a3444?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHxoZWFsdGhjYXJlJTIwZG9jdG9yJTIwaWxsdXN0cmF0aW9ufGVufDF8fHx8MTc3MjcyNDg4M3ww&ixlib=rb-4.1.0&q=80&w=1080',
  },
  {
    title: 'Đặt lịch khám nhanh chóng, tiện lợi',
    desc: 'Cho phép bệnh nhân đặt lịch, đổi lịch và hủy hẹn chỉ trong vài giây — giảm tình trạng vắng mặt và giải phóng nhân lực cho đội ngũ y tế.',
    img: 'https://images.unsplash.com/photo-1576091160399-112ba8d25d1d?w=800&q=80',
  },
  {
    title: 'Quản lý dữ liệu bệnh nhân an toàn',
    desc: 'Hồ sơ bệnh án tuân thủ chuẩn bảo mật với phân quyền truy cập theo vai trò. Đúng người, đúng thông tin, luôn luôn.',
...
function AnimatedButton({ onClick, loading, success, children, fullWidth = false, type = 'button' }) {
...
export default function AuthPage() {
  ...
  const [formData, setFormData] = useState({
    nationId: '', password: '', phoneNumber: '',
    fullName: '', email: '', gender: 'M', dob: ''
  });
  ...
  const handleNavigationByToken = (accessToken, loginMethod = 'local') => {
    if (!accessToken) return;
    try {
      // Decode JWT để lấy payload: role, fullName, nationId, walletAddress...
      const decoded = jwtDecode(accessToken);
      const role = decoded?.role?.toLowerCase();

      // Gộp thông tin user từ token để truyền sang dashboard
      const userState = {
        ...decoded,          // toàn bộ payload của token
        loginMethod,         // 'local' hoặc 'metamask'
      };

      // ── DEMO: luôn dẫn đến trang demo dashboard ──────────────────────────
      // TODO: bỏ comment block dưới và xóa dòng navigate('/demo-dashboard')
      // khi muốn điều hướng theo role thật
      navigate('/demo-dashboard', { state: { user: userState, loginMethod } });

      // ── PRODUCTION routing theo role ─────────────────────────────────────
      // if (!role || role === 'patient') navigate('/patient-info',      { state: { user: userState, loginMethod } });
      // else if (role === 'doctor')      navigate('/doctor-dashboard',   { state: { user: userState, loginMethod } });
      // else if (role === 'admin')       navigate('/admin-dashboard',    { state: { user: userState, loginMethod } });
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[40%_60%]">
          ...
          <div className="p-8 md:p-12">
            ...
            <form onSubmit={handleTraditionalAuth} className="space-y-5">
              ...
              {!isLoginMode && (
                <>
                  <InputField label="Họ và tên" name="fullName" type="text" placeholder="Nguyễn Văn A" icon={User} value={formData.fullName} onChange={handleInputChange} required minLength="2" />
                  <div className="grid grid-cols-1 md:grid-cols-[1fr_2fr] gap-4">
                    <div>
                      <label className="block text-sm font-bold text-black mb-2">Giới tính</label>
                      <div className="flex items-center border border-gray-300 rounded-lg overflow-hidden focus-within:ring-2 focus-within:ring-blue-500 bg-white">
                        <select name="gender" value={formData.gender} onChange={handleInputChange} className="w-full px-3 py-3 outline-none bg-transparent">
                          <option value="M">Nam</option>
                          <option value="F">Nữ</option>
                        </select>
                      </div>
                    </div>
                    <InputField label="Năm sinh" name="dob" type="number" placeholder="VD: 1990" icon={Calendar} value={formData.dob} onChange={handleInputChange} required />
                  </div>
                  <InputField label="Địa chỉ Email" name="email" type="email" placeholder="nguyenvana@gmail.com" icon={Mail} value={formData.email} onChange={handleInputChange} required />
                  <InputField label="Số điện thoại" name="phoneNumber" type="text" placeholder="0987654321" icon={Phone} value={formData.phoneNumber} onChange={handleInputChange} required pattern="(0|\+84)(3|5|7|8|9)[0-9]{8}" />
                </>
              )}

              {/* Shared fields */}
              <InputField label="Số CCCD/CMND" name="nationId" type="text" placeholder="Nhập số CCCD 12 chữ số hoặc CMND 9 chữ số" icon={CreditCard} value={formData.nationId} onChange={handleInputChange} required pattern="(\d{9}|\d{12})" />
              ...
              <div>
                ...
                <div className="flex items-center border border-gray-300 rounded-lg overflow-hidden focus-within:ring-2 focus-within:ring-blue-500 bg-white">
                  ...
                  <input
                    ...
                    className="flex-1 px-3 py-2.5 outline-none bg-transparent"
                  />
                  <button type="button" onClick={() => setShowPassword(!showPassword)} className="px-3 py-2.5 text-gray-400 hover:text-gray-600 transition">
              ...
              {isLoginMode && (
                <div className="flex items-center justify-between pt-2">
                  ...
                  <button type="button" className="text-sm text-emerald-500 font-medium hover:underline">
              ...
              <div className="relative py-4">
                ...
                <div className="relative flex justify-center text-sm">
                  <span className="px-4 bg-white text-gray-500">Hoặc kết nối tới ví điện tử của bạn</span>
                </div>
              </div>
              ...
              <div className="flex justify-center">
                ...
                >
                  <span className="flex items-center gap-3">
                    ...
                    />
                    <span style={{ color: metamaskBtn === 'idle' ? 'white' : 'transparent', transition: 'color 0.2s', fontWeight: 600 }}>
            ...
            <div className="mt-8 text-center text-sm">
              ...
              <button
                type="button"
                onClick={() => { setIsLoginMode(!isLoginMode); setLoginBtn('idle'); setMetamaskBtn('idle'); }}
